...
# ¿Qué pasa si hacemos una transformación logarítima de los datos? ¿Para qué sirve? Para que quede bonito
data2 = log2(data)
...
# ¿Qué es un boxplot? Un gráfico que muestra la distribución de un conjunto de datos a través de sus cuartiles
...
las=2 --> es para que los nombres nos salgan en vertical y no en horizontal por defecto	
...
--> nos muestra para comprobar que está bien
...
wt <- data[,1:3] ---> detrás de la coma van las columnas de la 1 a la 3 (en r empezamos a contar desde 1 y no desde 0 como en python)
ko <- data[,4:6] --> selecionamos las columnas de la 4 a la 6
class(wt) --> nos dice que es una matriz, una tabla...
...
abline(0, 1, col = "red") --> para añadir lineas

# ¿Eres capaz de añadirle un grid? --> un grid son los cuadraditos como los de la cámara
grid()
...
# OJO que aquí usamos los datos SIN TRANSFORMAR. ¿Por qué? Porque para hacer los análisis los haces con los datos brutos, sin transformar.
# ¿Cuántas valores tiene cada muestra? 6
pvalue = NULL 
tstat = NULL 
...
# ¿Qué pasa si le ponemos con una transformación de -log10? Que el corte de Pvalue de 0,05 equivale al dos
...
#Los puntitos negros son los genes que no pasan los criterios.
#Los puntos azules son los genes reprimmidos en WT.
#Los puntos rojos son los genes sobreexpresados en KO.
...
# ¿Eres capaz de cambiar los colores del heatmap? 
Pista: usar el argumento col y hcl.colors
